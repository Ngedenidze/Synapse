...
export const Pricing = () => (
  <motion.section
    id="pricing"
    className="flex items-center py-24 relative w-full overflow-visible min-h-[160vh]
  ...
  >
    <div className="container mx-auto px-6 lg:px-0">
      ...
      <div className="max-w-5xl mx-auto text-center mb-16">
        <h2 className="text-5xl md:text-6xl font-bold text-white font-azonix">
          Transparent, scalable <span className='text-purple-600 dark:text-purple-400'>web solutions</span>  built for growth
        </h2>
        <p className="mt-4 text-4xl font-azonix text-left text-gray-300">
        ...
        <br />
          <p className="mt-4 text-4xl leading-2 font-azonix text-left text-gray-300">
          At Synapse, we combine creativity, performance, and reliability to craft websites that elevate your brand.
          Choose a plan that aligns with your vision and unlock the power of modern web development.
        </p>
      </div>
      ...
      >
        {pricingTiers.map((tier) => (
          <motion.div
            key={tier.title}
            className={twMerge(
              'relative p-8 font-azonix rounded-3xl border shadow-lg flex flex-col h-[600px]',
              tier.highlight
          ...
          >
            {/* Title */}
            <h3 className="text-3xl  font-semibold text-purple-600 dark:text-purple-400">{tier.title}</h3>
            ...
            {/* Description */}
            <p className="mt-2 text-lg text-gray-400">{tier.description}</p>

            {/* CTA Button */}
            <button
              className={twMerge(
                'mt-6 w-full py-3 rounded-full font-azonix text-lg transition',
                tier.monthlyPrice === 0
            ...
            <ul className="mt-6 space-y-2 flex-1">
              {tier.features.map((f) => (
                <li key={f} className="flex items-center text-lg">
      ...
      <div className="flex mx-auto justify-center flex-col mt-20 text-center px-6 lg:px-0 text-4xl font-azonix max-w-5xl">
        ...
        <a
          href="#contact"
          className="relative inline-flex text-center justify-center mt-5 items-center w-full text-purple-600 dark:text-purple-400 hover:underline font-medium"
        >
